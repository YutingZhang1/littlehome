...
Page({
  ...
  refreshAvatarUrlFromFileID() {
    ...
    if (userInfo && userInfo.avatarFileID) {
      console.log('开始获取临时链接，fileID:', userInfo.avatarFileID);
      // 检查 fileID 格式
      if (!userInfo.avatarFileID.startsWith('cloud://')) {
        console.error('fileID 格式不正确:', userInfo.avatarFileID);
      ...
      wx.cloud.getTempFileURL({
        ...
        success: urlRes => {
          console.log('getTempFileURL 返回结果:', urlRes);
          console.log('fileList:', urlRes.fileList);
          if (urlRes.fileList && urlRes.fileList.length > 0) {
            const fileInfo = urlRes.fileList[0];
            console.log('第一个文件信息:', fileInfo);
            console.log('fileInfo.tempFileURL:', fileInfo.tempFileURL);
            console.log('fileInfo.status:', fileInfo.status);
            if (fileInfo && fileInfo.tempFileURL && fileInfo.tempFileURL.trim() !== '' && (!fileInfo.status || fileInfo.status === 0)) {
              ...
              console.log('成功获取临时链接:', fileInfo.tempFileURL);
            } else {
              console.error('文件信息中缺少 tempFileURL 或 status 不为0:', fileInfo);
              // 如果获取失败，使用默认头像
              const updatedUserInfo = {
                ...userInfo,
                avatarUrl: 'https://dis-1301530190.cos.ap-nanjing.myqcloud.com/LittlehomePic/profile.jpg'
            ...
            console.error('获取临时链接失败: fileList 为空或不存在:', urlRes);
            // 如果获取失败，使用默认头像
            const updatedUserInfo = {
              ...userInfo,
              avatarUrl: 'https://dis-1301530190.cos.ap-nanjing.myqcloud.com/LittlehomePic/profile.jpg'
      ...
      console.log('没有找到 avatarFileID，userInfo:', userInfo);
    }
  },
